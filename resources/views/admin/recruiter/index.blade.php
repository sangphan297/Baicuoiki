...
<div class="page">
    ...
    <section>
        <div class="container-fluid">
              <!-- Page Header-->
            <header> 
                <h1 class="h3 display">Recruiter Management</h1>
            </header>
            @if (Session::has('msg'))
                                       ...
                                       @foreach($users as $user)
                                        ...
                                        <tr>
                                            ...
                                            <td id="result-{{ $id_user }}">
                                                ...
                                                @elseif($active == 2)
                                                <center><p style="color: green;">Validating</p></center>
                                                @endif
                                                </form>
                                            </td>
                                            <td width="100px">
                                                <a href="{{route('admin.recruiter.del', $id_user)}}" title="" class="btn btn-danger"><i class="fa fa-pencil"></i> Delete</a>
                                            </td>
                                        </tr>
                                        @endforeach
                                        <script type="text/javascript">
                                            function changeStatus(id_user ,status_comment) {
                                                ...
                                                $.ajax({
                                                    url: '{{ route('admin.recruiter.method') }}',
                                                    type: 'POST',
                                                    data: {
                                                        id_user:id_user,
                                            ...
                                            function getInfo(id_user ,active) {
                                                var _token = $('input[name=_token]').val();
                                                $.ajax({
                                                    url: '{{ route('admin.recruiter.index') }}',
                                                    type: 'POST',
                                                    data: {
                                                        id_user:id_user,
                                                        active:active,
